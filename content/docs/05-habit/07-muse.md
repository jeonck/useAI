...
**바로 시작하기**
- Muse 열기: [muse.ai](https://muse.ai) — 가입 후 첫 대화에서 비서 이름을 정하면 끝. 설정 메뉴 없음
- 첫 메시지로 보낼 것: [케이스 1의 아침 브리핑 프롬프트](#케이스-1-아침-브리핑--우리-동네우리-업종-소식-10분) (컨텍스트 카드 + "매일 평일 아침 8시에")
- 앱 연결은 [케이스 3](#케이스-3-앱-연결--한-문장으로-시작-승인은-google-화면에서-10분)을 읽고 나서, 가게 캘린더만
{{< /callout >}}

{{< callout type="warning" >}}
**이 과정의 원칙은 여전히 "ChatGPT 하나만"입니다.** Muse([muse.ai](https://muse.ai))를 따로 다루는 이유는 하나입니다. **먼저 말을 겁니다.** ChatGPT는 사장님이 열어야 일하지만, Muse는 정해진 시간에 브리핑을 보내고, 며칠에 걸쳐 "이런 것도 할 수 있어요"를 먼저 제안합니다. [업무 비서 만들기](05-business-assistant)의 1층(지침)과 4층(예약)을 설정 메뉴 대신 대화로 처리합니다.
